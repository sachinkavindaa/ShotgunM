...
HUMAN_REF_DIR="/work/samodha/sachin/human_index"

# Create output directories
mkdir -p "$PHIX_REMOVED_DIR" "$REPAIR_DIR" "$TRIM_DIR" "$HUMAN_REMOVED_DIR" "$BASE_OUT/logs"
...
sickle pe -t sanger -f "$FW_REPAIRED" -r "$RV_REPAIRED" \
  -o "$FW_TRIMMED" -p "$RV_TRIMMED" -s "$SINGLETON" -q 30 -l 75
